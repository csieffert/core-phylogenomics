...
{	
	...
	#TODO: figure out how to accomplish the reordering with just the Bio::Tree::Tree module
	#parse the Bio::Tree::Tree object into a Bio::Phylo::Forest::Tree object to facilitate reordering
 	...
 	$self->updateMatrixCsv($tree);
 	#add the branch length to SNP labels to the tree nodes that show as: STRAIN[BRANCH_LENGTH, TOTAL_SNP's]
 	$self->branchLengthToSNP($tree);
 	#resize the tree to make more easily human readable
 	$self->resizeTree($tree, 0.1);

...
sub run {
	...
	$min_map=80 if (not defined $min_map);
...
#function to parse the output from nml_percentCoverage and check for low mapping percentages
sub parseMappingPercentage{
	my ($percentages, $logger) = @_;
	print "Getting here";
	my $csv = Text::CSV->new();
	print $csv->getline($percentages);
	print $logger "Accessing the log from the parse function!";
}
...
verify_mapping_quality.pl -l /log-direcotry -i /inputDataDirectory -m minimum-percent-mapping -g genome-size(bp) -h help
...
=item B<-m>, B<--min-percent> [optional]
...
verify_mapping_quality will check BAM files to ensure at least [MIN-PERCENT] of the isolate genome has been mapped to the reference sequence. 
